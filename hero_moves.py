...
class Camera(pygame.sprite.GroupSingle):
    ...
    def update(self):
        self.offset = self.sprite.rect.center - Vector2(App.width // 2, App.height // 2)

    def draw(self, screen, tiles):
        screen.fill('black')

        for tile in tiles.sprites():
            screen.blit(tile.image, tile.rect.center - self.offset)

        screen.blit(self.sprite.image, self.sprite.rect.topleft - self.offset)
        pygame.display.update()


class Hero(pygame.sprite.Sprite):
    def __init__(self, app, pos):
        super().__init__(app.camera, app.all_sprites)
...
class App:
    ...
    def run(self, map_path):
        ...
        while True:
            ...
            self.camera.draw(self.screen, self.tiles_group)
            self.clock.tick(self.fps)
